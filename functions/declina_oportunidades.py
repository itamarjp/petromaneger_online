...
def entraOportunidade(i):  # Funciona OK
    try:
        ...
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, page_selectors.num_oport)))
    except:
        wait = WebDriverWait(browser, 30)
        wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, page_selectors.num_oport)))
    campoOport = browser.find_element_by_css_selector(page_selectors.num_oport)
    campoOport.clear()
...
def clicaOport():  # Funciona Ok
    ...
    wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, page_selectors.clicaOportunidade)))
    clickOpt = browser.find_element_by_css_selector(page_selectors.clicaOportunidade)
    clickOpt.click()
...
def selecionaNulo():  # Funciona Ok
    ...
    eventoSelecao.click()
    time.sleep(3)
    nulo = browser.find_element_by_xpath(page_selectors.selecionaOpcaoNula)
    nulo.send_keys(Keys.ARROW_UP)
    nulo.send_keys(Keys.ARROW_UP)
    nulo.click()
...
def verif_id(i):  # Funciona Ok
    ...
    try:
        j = i.rstrip()
        buscaId = r"""(//a[@lsdata="{0:'N\xfamero',1:'""" + j + """',4:true}"])"""
        checaID = browser.find_element_by_xpath(buscaId)
        checaIdAtributo = checaID.get_attribute("lsdata")
        checkId = j in checaIdAtributo
        if checkId:
...
def downloadFilesProcess(listIds, loginPetronect, SenhaPetronect, username, titleList, idArticles, id):
    global browser
    render_template('downloadInProgress.html')
    pasta = criarPastaUser(username, titleList)
...
def criarPastaUser(username, titleList):
    pasta = f"C:\\PetroBot\\{username}\\{titleList}"
